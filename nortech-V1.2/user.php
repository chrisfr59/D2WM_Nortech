...
function rechercher1($name){
    global $connexion;
    $existant= false;
    $req1 = $connexion->prepare("SELECT i.*,e.nom,e.prenom FROM utilisateur i,employe e where i.noemp=e.noEmp AND e.nom='$name'  ");
...
<section id="section1">
  ...
  <?php
  include('db_connect.php');
 $sql="SELECT i.*,e.nom,e.prenom,e.noEmp FROM utilisateur i,employe e  where i.noEmp=e.noEmp";
